...
/****** Object:  StoredProcedure [App].[nprod_CalculateCareSitesVastRadius]    Script Date: 7/2/2019 4:42:36 PM ******/
DROP PROCEDURE [App].[nprod_ProcessDEASchedulingPrivilegesVistaData]
GO
--
DROP PROCEDURE [App].[nprod_ProcessMedicalEducationVistaData]
GO
--
DROP PROCEDURE [App].[nprod_ProcessOtherIdentifierVistaData]
GO
--
DROP PROCEDURE [App].[nprod_ProcessProviderServicesVistaData]
GO
--
DROP PROCEDURE [App].[nprod_ProcessProviderVistaData]
GO
--
SET ANSI_NULLS ON
GO
...
GO
--
CREATE PROCEDURE [App].[nprod_ProcessDEASchedulingPrivilegesVistaData]
AS

DECLARE @T TABLE([ProviderDEASchedulePrivilege_stg_Id] INT);

MERGE [App].[nprod_ProviderDEASchedulePrivilege_V] AS T
USING [App].[nprod_ProviderDEASchedulePrivilege_stg_V] AS S
...
DELETE [App].[nprod_ProviderDEASchedulePrivilege_stg_V] WHERE ProviderDEASchedulePrivilege_stg_Id in (SELECT ProviderDEASchedulePrivilege_stg_Id FROM @T);
GO
--
--
CREATE PROCEDURE [App].[nprod_ProcessMedicalEducationVistaData]
AS

DECLARE @T TABLE([ProviderMedicalEducation_stg_Id] INT);

MERGE [App].[nprod_ProviderMedicalEducation_V] AS T
USING [App].[nprod_ProviderMedicalEducation_stg_V] AS S
...
DELETE [App].[nprod_ProviderMedicalEducation_stg_V] WHERE ProviderMedicalEducation_stg_Id in (SELECT ProviderMedicalEducation_stg_Id FROM @T);
GO
--
--
CREATE PROCEDURE [App].[nprod_ProcessOtherIdentifierVistaData]
AS

DECLARE @T TABLE([ProviderOtherIdentifier_stg_Id] INT);

MERGE [App].[nprod_ProviderOtherIdentifier_V] AS T
USING [App].[nprod_ProviderOtherIdentifier_stg_V] AS S
...
DELETE [App].[nprod_ProviderOtherIdentifier_stg_V] WHERE ProviderOtherIdentifier_stg_Id in (SELECT ProviderOtherIdentifier_stg_Id FROM @T);
GO
--
--
CREATE Procedure [App].[nprod_ProcessProviderServicesVistaData] AS

Update Staging Set
Staging.NvaProvider_Id_fk=NVA.NonVAProvider_Id
from APP.nprod_ProviderService_stg_V Staging
inner join App.nprod_NonVAProvider_V NVA on (Staging.ProviderNpi=NVA.ProviderNpi)

Delete from App.nprod_ProviderService_stg_V where NvaProvider_Id_fk is null

update App.nprod_ProviderService_stg_V set IsPrimaryTaxonomy=null where IsPrimaryTaxonomy='null'

Insert into App.nprod_CareSite_V (CareSiteGuid, CareSiteType, CareSiteName, AddressComposite, AddressStreet1,
...
getdate()
from APP.nprod_ProviderService_stg_V Staging
Left outer join App.nprod_CareSite_V CareSite on (Staging.CareSiteGuid=CareSite.CareSiteGuid)
...
from APP.nprod_ProviderService_stg_V Staging
inner join App.nprod_CareSite_V CareSite on (Staging.CareSiteGuid=CareSite.CareSiteGuid)
...
Insert into App.nprod_ProviderServiceCareSite_V (NonVAProvider_Id_FK,CareSite_Id_FK,SpecialityCode,IsPrimaryTaxonomy, ProviderServiceStatus,
...
'PIE',
 getdate()
From APP.nprod_ProviderService_stg_V Staging
Left outer join App.nprod_ProviderServiceCareSite_V PS 
...
From App.nprod_ProviderServiceCareSite_V PS
inner join  APP.nprod_ProviderService_stg_V Staging  
	on ( Staging.NvaProvider_Id_fk= PS.NonVAProvider_Id_FK and 	Staging.CareSite_Id_Fk=PS.CareSite_Id_Fk and
		 Staging.SpecialtyCode=PS.SpecialityCode and Staging.ProviderNetwork=PS.ProviderNetwork)
...
--
--
CREATE Procedure [App].[nprod_ProcessProviderVistaData] AS
	
Insert  into APP.nprod_NonVAProvider_V (ProviderNpi,ProviderType,ProviderFirstName,ProviderMiddleName,
...
'PIE',
getdate()
from APP.nprod_NonVAProvider_stg_V Staging
left outer join App.nprod_NonVAProvider_V NVA on (Staging.ProviderNpi=NVA.ProviderNpi)
