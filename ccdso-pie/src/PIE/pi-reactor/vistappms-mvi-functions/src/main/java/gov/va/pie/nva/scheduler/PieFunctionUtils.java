...
import gov.va.pie.nva.scheduler.AadToken;
...
public class PieFunctionUtils {
	...
	public static final String SERVER_URL = "https://nva.ppms.va.gov/";

	public static final String MVI_ENDPOINT_URL = SERVER_URL+"pie/mvi/triggers/outbound";
	public static final String PPMS_ENDPOINT_URL = SERVER_URL+"pie/ppms/triggers/nvappmscdw";
	...
	public String triggerEndPoint(ExecutionContext context, String serverUrl, String endPoint) throws IOException {
		...
		if (secret == null || secret.equals("")) {
			...
			AadToken validatedAccessToken = getValidatedToken(context, accessToken, serverUrl);
			...
			if (httpStatus == org.springframework.http.HttpStatus.OK && !StringUtils.isEmpty(validatedAccessToken)) {
				...
				ResponseEntity<String> serviceResponse = serviceRestTemplate.exchange(endPoint,
						org.springframework.http.HttpMethod.POST, serviceEntity, String.class);
				...
				if (serviceHttpStatus == org.springframework.http.HttpStatus.OK) {
					...
					httpClient.close();
					return "Calling PIE web service failed.";
				}
	...
	private AadToken getValidatedToken(ExecutionContext context, AadToken token, String URL) {
		...
		if (!TOKEN_TYPE.equals(token.getToken_type())) {
			...
			return null;
		} else if (!URL.equals(token.getResource())) {
