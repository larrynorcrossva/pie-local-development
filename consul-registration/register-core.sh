...
curl -H "X-Consul-Token: ${CONSUL_MASTER_TOKEN}" -s -X PUT ${HOST_DOMAIN}:8500/v1/kv/vamf/${VAMF_ENVIRONMENT}/apigateway/1.0/services/scheduling-service -d '{ "location":"/VistaSchedulingService", "service":"vista-scheduling-service", "redirect":"off",  "request_headers":"on", "headers": { "X-Real-IP":"$remote_addr","X-Forwarded-Proto": "https", "X-Forwarded-Host" : "$http_host" }}'  > /dev/null && \
